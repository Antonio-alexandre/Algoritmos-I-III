...
int main()
{
    ...
    for (int i = 0; i < numReservas; ++i) {
        ...
        ponto v[100];
        lerReserva(v, numArvores);

        list<ponto> fecho_convexo = Graham_scan(v, numArvores);
        
        ponto anterior = fecho_convexo.back();
        list<ponto>::iterator it = fecho_convexo.begin();

        while (it != fecho_convexo.end()) {
            ponto atual = *it;
            totalMaterial += sqrt(pow(atual.x - anterior.x, 2) + pow(atual.y - anterior.y, 2));
            anterior = atual;
            it++;
        }
    }

    cout << fixed << setprecision(4) << totalMaterial << endl;
